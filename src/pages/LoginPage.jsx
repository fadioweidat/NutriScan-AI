...
export default function LoginPage() {
  ...
  const { signIn, signUp, resetPasswordForEmail, user } = useAuth();
   ...
   const validate = () => {
     ...
     if (activeTab === 'forgot') return true;
     if (!password) {
   ...
   const handleSubmit = async (e) => {
     ...
     try {
       if (activeTab === 'login') {
         ...
         setSuccessMsg('Registrazione completata! Se richiesto, controlla la tua casella e-mail per confermare l\'account.');
       } else {
         const { error: resetError } = await resetPasswordForEmail(email.trim());
         if (resetError) {
           setError(resetError.message || 'Errore durante l\'invio dell\'email. Riprova.');
         } else {
           setSuccessMsg('Email di ripristino password inviata! Controlla la tua casella di posta.');
         }
       }
     } catch (err) {
         ...
         <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-2xl">
           ...
           {activeTab === 'forgot' ? (
             <div className="flex items-center justify-between mb-6">
               <h2 className="text-white font-bold text-base">Reimposta Password</h2>
               <button
                 type="button"
             ...
             >
               {loading ? (
                 <>
                   ...
                   {activeTab === 'login' ? 'Accesso in corso...' : activeTab === 'register' ? 'Registrazione in corso...' : 'Invio in corso...'}
                 </>
               ) : (
                 activeTab === 'login' ? 'Accedi' : activeTab === 'register' ? 'Crea Account' : 'Invia Link di Ripristino'
               )}
             </button>
           </form>
        {/* Footer text */}
